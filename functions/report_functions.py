...
def generate_results_percentiles(results, model_parameters):
    ...
    for i in range(len(model_parameters.isolation_level)):
        ...
        HmaxTemp = []
        UmaxTemp = []
        HmaxTemp.append(np.median(np.max(Hi+Hj,axis=1)))
        HmaxTemp.append(np.quantile(np.max(Hi+Hj,axis=1), 0.05))
        HmaxTemp.append(np.quantile(np.max(Hi+Hj,axis=1), 0.95))
        Hmax.append(HmaxTemp)
        UmaxTemp.append(np.median(np.max(Ui+Uj,axis=1)))
        UmaxTemp.append(np.quantile(np.max(Ui+Uj,axis=1), 0.05))
        UmaxTemp.append(np.quantile(np.max(Ui+Uj,axis=1), 0.95))
        Umax.append(UmaxTemp)
    ...
    return results_percentiles, Hmax, Umax
